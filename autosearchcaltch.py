...
against Caltech-101 targets, then recursively layer more patches on top.

Pipeline:
  1. Loads patch images from PATCH_DIR, removes background
  2. Patches are matched against a target pool from Caltech-101 (level 0)
...
def load_caltech101_targets(num_targets, split="test"):
    """Load random targets from Caltech-101 via HuggingFace datasets streaming."""
    print(f"Loading {num_targets} targets from Caltech-101 ({split})...")
    ds = load_dataset("clip-benchmark/wds_vtab-caltech101", split=split, streaming=True)

    # Peek at the first item to discover the correct key names
    ds_iter = iter(ds)
    first = next(ds_iter)
    ...
    # Detect image key
    image_key = None
    for candidate in ("jpg", "image", "png", "webp", "pixel_values"):
        if candidate in first:
            image_key = candidate
            break
    if image_key is None:
        # Fall back to first PIL-image-valued key
    ...
    # Detect label key
    label_key = None
    for candidate in ("cls", "label", "class", "target", "fine_label"):
        if candidate in first:
            label_key = candidate
            break
    ...
    def extract(item):
        ...
        return {
            ...
            "class_name": f"class_{item[label_key]}",
        }

    # Reservoir sample — include the already-peeked first item
    samples = [extract(first)]
    for i, item in enumerate(ds_iter):
        idx = i + 1  # account for the peeked item
    ...
    # Resolve integer labels to string names
    try:
        features = load_dataset(
            "clip-benchmark/wds_vtab-caltech101", split=split, streaming=True
        ).features
        label_feature = features[label_key]
        for t in samples:
            t["class_name"] = label_feature.int2str(t["label"])
    except Exception:
        pass  # keep class_N fallback

    print(f"  {len(samples)} targets loaded")
    return samples
def precompute_outlines_caltech(targets, max_resolution, num_threads):
    """Compute subject outlines for Caltech-101 targets."""
...
if __name__ == "__main__":
    ...
    CALTECH_SPLIT = "test"   # "train" or "test"
    ...
    print(f"  Caltech-101 split:{CALTECH_SPLIT}")
    ...
    # ----------------------------------------------------------
    # Step 2: load Caltech-101 targets
    # ----------------------------------------------------------
    targets = load_caltech101_targets(NUM_TARGETS, split=CALTECH_SPLIT)

    # ----------------------------------------------------------
    # Step 3: precompute target outlines
    # ----------------------------------------------------------
    print(f"Precomputing outlines for {len(targets)} targets...")
    targets_processed = precompute_outlines_caltech(
    ...
    for i, p in enumerate(accepted):
        ...
        if matches:
            best = matches[0]
            print(f"  {i+1}. {p['class_name']} -> {best['class_name']} (score={best['score']:.3f})")
        else:
